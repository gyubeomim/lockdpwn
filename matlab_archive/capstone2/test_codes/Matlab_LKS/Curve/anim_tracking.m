function anim_tracking(P,Vx,X,Y,psi,LO,t,scale,steerangle,nframe,imanual)
...
while 1
    ...
    s4 = ['steerangle = ',num2str(steerangle(k)*180/pi*17.25), ' [deg]'];
    ...
    xlabel('time [sec]');ylabel('Lateral Offset [m]');
    axis([t(1) t(end) min(LO)*1.1 max(LO)*1.1])
    
    subplot(2,2,4,'Linewidth',2)
    plot(t(1:k),steerangle(1:k)*180/pi*17.25);
    title('Steering Angle')
    xlabel('time [sec]');ylabel('SWA [deg]');
    axis([t(1) t(end) min(steerangle*180/pi*17.25)*1.1 max(steerangle*180/pi*17.25)*1.1])
