...
static real_T c3_emlrt_marshallIn(SFc3_vehicle_longitudinal_modelInstanceStruct *
  chartInstance, const mxArray *c3_sign, const char_T *c3_identifier);
...
static uint8_T c3_d_emlrt_marshallIn
  (SFc3_vehicle_longitudinal_modelInstanceStruct *chartInstance, const mxArray
   *c3_b_is_active_c3_vehicle_longitudinal_model, const char_T *c3_identifier);
  ...
  (SFc3_vehicle_longitudinal_modelInstanceStruct *chartInstance)
{
  ...
  real_T *c3_sign;
  c3_sign = (real_T *)ssGetOutputPortSignal(chartInstance->S, 1);
  c3_st = NULL;
  c3_st = NULL;
  c3_y = NULL;
  sf_mex_assign(&c3_y, sf_mex_createcellarray(2), FALSE);
  c3_hoistedGlobal = *c3_sign;
  c3_u = c3_hoistedGlobal;
  c3_b_y = NULL;
  sf_mex_assign(&c3_b_y, sf_mex_create("y", &c3_u, 0, 0U, 0U, 0U, 0), FALSE);
  sf_mex_setcell(c3_y, 0, c3_b_y);
  c3_b_hoistedGlobal = chartInstance->c3_is_active_c3_vehicle_longitudinal_model;
   ...
   *c3_st)
{
  ...
  *c3_sign = c3_emlrt_marshallIn(chartInstance, sf_mex_dup(sf_mex_getcell(c3_u,
    0)), "sign");
  ...
  (SFc3_vehicle_longitudinal_modelInstanceStruct *chartInstance)
{
  ...
  real_T c3_sign;
  real_T *c3_b_slip_ratio;
  real_T *c3_b_sign;
  boolean_T guard1 = FALSE;
  c3_b_sign = (real_T *)ssGetOutputPortSignal(chartInstance->S, 1);
  c3_b_slip_ratio = (real_T *)ssGetInputPortSignal(chartInstance->S, 0);
  _SFD_SYMBOL_SCOPE_PUSH(0U, 0U);
  _sfTime_ = (real_T)ssGetT(chartInstance->S);
  _SFD_CC_CALL(CHART_ENTER_SFUNCTION_TAG, 2U, chartInstance->c3_sfEvent);
  _SFD_DATA_RANGE_CHECK(*c3_b_slip_ratio, 0U);
  _SFD_DATA_RANGE_CHECK(*c3_b_sign, 1U);
  chartInstance->c3_sfEvent = CALL_EVENT;
  _SFD_CC_CALL(CHART_ENTER_DURING_FUNCTION_TAG, 2U, chartInstance->c3_sfEvent);
  c3_hoistedGlobal = *c3_b_slip_ratio;
  c3_slip_ratio = c3_hoistedGlobal;
  ...
  _SFD_SYMBOL_SCOPE_ADD_EML_IMPORTABLE(&c3_nargout, 1U, c3_sf_marshallOut,
    c3_sf_marshallIn);
  _SFD_SYMBOL_SCOPE_ADD_EML(&c3_slip_ratio, 2U, c3_sf_marshallOut);
  _SFD_SYMBOL_SCOPE_ADD_EML_IMPORTABLE(&c3_sign, 3U, c3_sf_marshallOut,
    c3_sf_marshallIn);
  CV_EML_FCN(0, 0);
  _SFD_EML_CALL(0U, chartInstance->c3_sfEvent, 3);
  c3_sign = 1.0;
  _SFD_EML_CALL(0U, chartInstance->c3_sfEvent, 5);
  if (CV_EML_IF(0, 1, 0, c3_slip_ratio >= -0.1)) {
    _SFD_EML_CALL(0U, chartInstance->c3_sfEvent, 6);
    c3_sign = -1.0;
  } else {
    _SFD_EML_CALL(0U, chartInstance->c3_sfEvent, 8);
    guard1 = FALSE;
    if (CV_EML_COND(0, 1, 0, c3_slip_ratio <= -0.3)) {
      if (CV_EML_COND(0, 1, 1, c3_slip_ratio >= -5.0)) {
    ...
    if (guard1 == TRUE) {
      ...
      if (CV_EML_IF(0, 1, 2, c3_slip_ratio <= -5.0)) {
  ...
  chartInstance, const mxArray *c3_sign, const char_T *c3_identifier)
{
  ...
  c3_y = c3_b_emlrt_marshallIn(chartInstance, sf_mex_dup(c3_sign), &c3_thisId);
  sf_mex_destroy(&c3_sign);
  return c3_y;
}
  ...
  *c3_mxArrayInData, const char_T *c3_varName, void *c3_outData)
{
  ...
  chartInstance = (SFc3_vehicle_longitudinal_modelInstanceStruct *)
    chartInstanceVoid;
  c3_sign = sf_mex_dup(c3_mxArrayInData);
   ...
   *c3_b_is_active_c3_vehicle_longitudinal_model, const char_T *c3_identifier)
{
  ...
  c3_y = c3_e_emlrt_marshallIn(chartInstance, sf_mex_dup
    (c3_b_is_active_c3_vehicle_longitudinal_model), &c3_thisId);
  sf_mex_destroy(&c3_b_is_active_c3_vehicle_longitudinal_model);
  return c3_y;
}
...
static const mxArray *sf_get_sim_state_info_c3_vehicle_longitudinal_model(void)
{
  ...
  const char *infoEncStr[] = {
    "100 S1x2'type','srcId','name','auxInfo'{{M[1],M[5],T\"sign\",},{M[8],M[0],T\"is_active_c3_vehicle_longitudinal_model\",}}"
  };
    ...
    if (ssIsFirstInitCond(S) && fullDebuggerInitialization==1) {
      /* do this only if simulation is starting */
      {
...

        {
          ...
          _SFD_SET_DATA_VALUE_PTR(1U, c3_sign);
        }
      }
    } else {
