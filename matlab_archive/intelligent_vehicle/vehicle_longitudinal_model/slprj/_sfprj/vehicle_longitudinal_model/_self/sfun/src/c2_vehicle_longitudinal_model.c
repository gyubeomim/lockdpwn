...
static real_T c2_emlrt_marshallIn(SFc2_vehicle_longitudinal_modelInstanceStruct *
  chartInstance, const mxArray *c2_sign, const char_T *c2_identifier);
...
static uint8_T c2_d_emlrt_marshallIn
  (SFc2_vehicle_longitudinal_modelInstanceStruct *chartInstance, const mxArray
   *c2_b_is_active_c2_vehicle_longitudinal_model, const char_T *c2_identifier);
  ...
  (SFc2_vehicle_longitudinal_modelInstanceStruct *chartInstance)
{
  ...
  real_T *c2_sign;
  c2_sign = (real_T *)ssGetOutputPortSignal(chartInstance->S, 1);
  c2_st = NULL;
  c2_st = NULL;
  c2_y = NULL;
  sf_mex_assign(&c2_y, sf_mex_createcellarray(2), FALSE);
  c2_hoistedGlobal = *c2_sign;
  c2_u = c2_hoistedGlobal;
  c2_b_y = NULL;
  sf_mex_assign(&c2_b_y, sf_mex_create("y", &c2_u, 0, 0U, 0U, 0U, 0), FALSE);
  sf_mex_setcell(c2_y, 0, c2_b_y);
  c2_b_hoistedGlobal = chartInstance->c2_is_active_c2_vehicle_longitudinal_model;
   ...
   *c2_st)
{
  ...
  *c2_sign = c2_emlrt_marshallIn(chartInstance, sf_mex_dup(sf_mex_getcell(c2_u,
    0)), "sign");
  ...
  (SFc2_vehicle_longitudinal_modelInstanceStruct *chartInstance)
{
  ...
  real_T c2_sign;
  real_T *c2_b_slip_ratio;
  real_T *c2_b_sign;
  boolean_T guard1 = FALSE;
  c2_b_sign = (real_T *)ssGetOutputPortSignal(chartInstance->S, 1);
  c2_b_slip_ratio = (real_T *)ssGetInputPortSignal(chartInstance->S, 0);
  _SFD_SYMBOL_SCOPE_PUSH(0U, 0U);
  _sfTime_ = (real_T)ssGetT(chartInstance->S);
  _SFD_CC_CALL(CHART_ENTER_SFUNCTION_TAG, 1U, chartInstance->c2_sfEvent);
  _SFD_DATA_RANGE_CHECK(*c2_b_slip_ratio, 0U);
  _SFD_DATA_RANGE_CHECK(*c2_b_sign, 1U);
  chartInstance->c2_sfEvent = CALL_EVENT;
  _SFD_CC_CALL(CHART_ENTER_DURING_FUNCTION_TAG, 1U, chartInstance->c2_sfEvent);
  c2_hoistedGlobal = *c2_b_slip_ratio;
  c2_slip_ratio = c2_hoistedGlobal;
  ...
  _SFD_SYMBOL_SCOPE_ADD_EML_IMPORTABLE(&c2_nargout, 1U, c2_sf_marshallOut,
    c2_sf_marshallIn);
  _SFD_SYMBOL_SCOPE_ADD_EML(&c2_slip_ratio, 2U, c2_sf_marshallOut);
  _SFD_SYMBOL_SCOPE_ADD_EML_IMPORTABLE(&c2_sign, 3U, c2_sf_marshallOut,
    c2_sf_marshallIn);
  CV_EML_FCN(0, 0);
  _SFD_EML_CALL(0U, chartInstance->c2_sfEvent, 3);
  c2_sign = 1.0;
  _SFD_EML_CALL(0U, chartInstance->c2_sfEvent, 5);
  if (CV_EML_IF(0, 1, 0, c2_slip_ratio >= -0.1)) {
    _SFD_EML_CALL(0U, chartInstance->c2_sfEvent, 6);
    c2_sign = -1.0;
  } else {
    _SFD_EML_CALL(0U, chartInstance->c2_sfEvent, 8);
    guard1 = FALSE;
    if (CV_EML_COND(0, 1, 0, c2_slip_ratio <= -0.3)) {
      if (CV_EML_COND(0, 1, 1, c2_slip_ratio >= -5.0)) {
    ...
    if (guard1 == TRUE) {
      ...
      if (CV_EML_IF(0, 1, 2, c2_slip_ratio <= -5.0)) {
  ...
  chartInstance, const mxArray *c2_sign, const char_T *c2_identifier)
{
  ...
  c2_y = c2_b_emlrt_marshallIn(chartInstance, sf_mex_dup(c2_sign), &c2_thisId);
   ...
   *c2_b_is_active_c2_vehicle_longitudinal_model, const char_T *c2_identifier)
{
  ...
  c2_y = c2_e_emlrt_marshallIn(chartInstance, sf_mex_dup
    (c2_b_is_active_c2_vehicle_longitudinal_model), &c2_thisId);
  sf_mex_destroy(&c2_b_is_active_c2_vehicle_longitudinal_model);
  return c2_y;
}
...
static const mxArray *sf_get_sim_state_info_c2_vehicle_longitudinal_model(void)
{
  ...
  const char *infoEncStr[] = {
    "100 S1x2'type','srcId','name','auxInfo'{{M[1],M[5],T\"sign\",},{M[8],M[0],T\"is_active_c2_vehicle_longitudinal_model\",}}"
  };
    ...
    if (ssIsFirstInitCond(S) && fullDebuggerInitialization==1) {
      /* do this only if simulation is starting */
      {
...

        {
          ...
          _SFD_SET_DATA_VALUE_PTR(1U, c2_sign);
        }
      }
    } else {
