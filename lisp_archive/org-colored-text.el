       ...
       (if rgb
           (progn
             (setq r (* 255 (/ (nth 1 rgb) 65535.0))
                   g (* 255 (/ (nth 2 rgb) 65535.0))
                   b (* 255 (/ (nth 3 rgb) 65535.0)))
             (format "<span style=\"color: rgb(%s,%s,%s);font-weight:bold\">%s</span>"
                     (truncate r) (truncate g) (truncate b)
                     (or description color)))
         (format "No Color RGB for %s" color)))))))
...
(defun next-color-bold-link (limit)
  (when (re-search-forward
	 "colorbold:[a-zA-Z]\\{2,\\}" limit t)
    (forward-char -2)
    (let* ((next-link (org-element-context))
	   color beg end post-blanks)
      (if next-link
	  (progn
		...
		(- end post-blanks)
                'color t
                'face
                `((:foreground ,color :underline nil :weight bold)))
	    (goto-char end))
	(goto-char limit)
	nil))))
...
(add-hook 'org-mode-hook
	  (lambda ()
	    (font-lock-add-keywords
	     nil
	     '((next-color-bold-link (0 'org-link t)))
	     t)))
