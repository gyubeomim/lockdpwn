...
# ChangeLog: https://code.google.com/p/peframe/wiki/ChangeLog
...
def PEID():
        signatures = peutils.SignatureDatabase(pathname + os.sep + 'modules' + os.sep + 'userdb.txt')
...
def HELP():
		...
		print "\t\t--file-url\tExtract File Name and Url"
		print "\t\t--file-verbose\tDiscover potential file name"
...
if len(sys.argv) < 3:
		...
		try:
			...
			if opt == '-h' or opt == '--help':
				...
				STRINGS()
			elif opt == '--file-url' or opt == '--file-verbose':
				URL()
			elif opt == '--suspicious':
				SUSPICIOUS()
			elif opt == '--dump':
