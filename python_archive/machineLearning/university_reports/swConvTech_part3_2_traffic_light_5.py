...
# Training Finished
#----------------------------------------------------------------
# Test for Ranking, Part3_2 코드 추가 
#----------------------------------------------------------------
import time 

base_test_path = "E:\\gitrepo\\lockdpwn\\python_archive\\ipython\\swConvTech_part3_2\\deep_learning_traffic_light_20101447\\test_images"

test_set2 = []

for im_type in image_types:
    for ex in glob.glob(os.path.join(base_test_path, im_type, "*")):
        im = cv2.imread(ex)
        if not im is None:
            im = cv2.resize(im, (32, 32))
            ...
            test_set2.append((im, one_hot_array, ex))

test_x2, test_y2, test_z2 = zip(*test_set2)
zipped_x_y = list(zip(test_x2, test_y2))
conf_true = []
conf_pred = []
total_count = 0
correct_count = 0
...
with tf.Session() as sess2:
    saver.restore(sess2, checkpoint_name)
#    optimistic_restore(sess2, checkpoint_name)
    print("Model restored.")

    for tt in range(0, len(zipped_x_y)):
        q = zipped_x_y[tt]
        sfmax = list(sess2.run(tf.nn.softmax(y.eval(feed_dict={x: [q[0]]})))[0])
