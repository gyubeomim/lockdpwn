...
;;
;; 2013-01-10 (1.2)
;;    `highlight-symbol-color' may now contain faces in addition to colors.
...
;;    Replaced `highlight-symbol-faces' with `highlight-symbol-color'.
...
(defcustom highlight-symbol-color
  '("yellow" "DeepPink" "cyan" "white" "SpringGreen1"
  ...
  (let ((color (nth highlight-symbol-color-index
    ...
    (if color ;; wrap
      ...
      (setq highlight-symbol-color-index 1
            color (car highlight-symbol-color)))
    color))
