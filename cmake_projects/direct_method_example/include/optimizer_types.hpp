...
void GeneralOptMarginals<T>::marginalize_frame(OptNode<T>* frame, bool delete_edges){
    ...
    const auto get_zero_hessian = [dim](){ return Eigen::Matrix<T,-1,-1>::Zero(dim,dim);};
    const auto get_f_other = [frame](std::pair<OptNode<T>*,OptNode<T>*> pair){
        OptNode<T>* f_other = pair.first!=frame? pair.first : pair.second;
        return f_other;
    };
    // ff_edge 체크
    for(auto it = frame->edges().begin();
            it != frame->edges().end(); it++){
        OptEdge<T>* e = *it;
    ...
    for(auto it_edge = ff_edges.begin();
            it_edge != ff_edges.end(); it_edge++) {
        OptEdge<T>* e = *it_edge;
    ...
    for(auto it = dUjk_.begin(); it != dUjk_.end(); it++){
        ...
        bool c1 = fa == frame;
        bool c2 = fb == frame;
        if(!c1 && !c2) continue; // 독립
        if(!c1) f_reserved.insert(fa);
        if(!c2) f_reserved.insert(fb);
        if(c1 && c2) continue;
...
void GeneralOptMarginals<T>::marginalize_point(OptNode<T>* pt, bool delete_edges){
    ...
    const auto p_edge_get_jbres
        = [](OptEdge<T>* e) -> const Eigen::Matrix<T,-1,1>& { return e->jres().at(0); };
    ...
    const auto fp_edge_get_jbres
        = [](OptEdge<T>* e) -> const Eigen::Matrix<T,-1,1>& { return e->jres().at(1); };
    ...
    for(auto it_e1 = fp_edges.begin();
            it_e1 != fp_edges.end(); it_e1++){
        OptEdge<T>* e1 = *it_e1;
        OptNode<T>* f1 =  get_frame(e1);
        for(auto it_e2 = it_e1;
                it_e2 != fp_edges.end(); it_e2++){
            OptEdge<T>* e2 = *it_e2;
        ...
        if(!jres_.count(f1)) jres_[f1] = Eigen::Matrix<T,-1,1>::Zero(f1->dim());
...
template<typename T>
ProjSE3Edge<T>::ProjSE3Edge(PoseSE3<T>* f, Point3<T>* pt,
