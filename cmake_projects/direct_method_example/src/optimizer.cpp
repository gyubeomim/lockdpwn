...
void se3_marginalization_test()
{
  ...
  auto donut_generate = [&generator,&xy_distribution]()->T
                        {
                          T v;
                          while(1)
                          {
...
void optimizer_test()
{
  ...
  std::vector<TestCase> test_list = {BA_OPTIMIZATION, SE2_MARGINALIZATION, SE3_MARGINALIZATION};

  for(auto it = test_list.begin(); it!=test_list.end(); it++)
  {
    if(*it == SE2_MARGINALIZATION) {
      std::cout << std::endl << "[+] SE2_MARGINALIZATION " << std::endl;
      se2_marginalization_test<T>();
    }
    else if(*it == SE3_MARGINALIZATION) {
      std::cout << std::endl << "[+] SE3_MARGINALIZATION " << std::endl;
      se3_marginalization_test<T>();
    }
    else {
      std::cout << std::endl << "[+] test_optimize<T> " << std::endl;
      test_optimize<T>(*it);
    }
  }
