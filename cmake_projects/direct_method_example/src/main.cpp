...
void loop_detect_test(){
    printf("TODO Loop detection doesn't work yet.\n");
    exit(1);
    std::map<size_t, std::string> im0_filenames, im1_filenames;
...
int main(int argc, char* argv[]){
    if(argc > 1){
        ...
        if(std::string("invd").compare(str)==0)
            ...
            motion_test();
        else if(std::string("loop").compare(str)==0)
            loop_detect_test();
        else if(std::string("shuttum").compare(str)==0)
            shuttum_test();
        else if(std::string("ris").compare(str)==0)
            ris_test();
        else if(std::string("shuttum_rect").compare(str)==0)
            shuttum_rectify_test();
        else if(std::string("ris_rect").compare(str)==0)
            ris_rectify_test();
        else if(std::string("gamma").compare(str)==0)
            gamma_test();
        else if(std::string("distortion").compare(str)==0)
            distortion_test();
        else if(std::string("graph").compare(str)==0)
            graph_test();
        else if(std::string("refac").compare(str)==0)
            refac_test();
        else // lba
            lba_test();
    }
