...
void Convolution(int input_width, uchar* input_img,
                 int kernel_width, int kernel_height, double* kernel,
      ...
      for (int m = 0; m < kernel_width; ++m) {
        for (int n = 0; n < kernel_height; ++n) {
          for (int channel = 0; channel < num_channels; ++channel)
            output_img[num_channels * (i + output_width * j) + channel] += uchar(input_img[num_channels * ((k - m) + input_width * (l - n)) + channel] * kernel[m + kernel_width * n]);
        }
      }
    }
  }
...
int main(int argc, char* argv[]) {
  ...
  int kernel_width = 5, kernel_height = 3;

  double* kernel = new double[kernel_width * kernel_height];

  for (int i = 0; i < kernel_width; ++i)
    for (int j = 0; j < kernel_height; ++j)
      kernel[i + kernel_width * j] = .5f;
  ...
  Convolution(input_width, input_img.data, kernel_width, kernel_height, kernel, output_width, output_height, output_img.data);
