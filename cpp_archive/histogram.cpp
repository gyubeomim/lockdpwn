...
histogram::histogram(double* data, int size, double binsize)
...
histogram::histogram(double* data, int size, double binsize, char* fname)
    :size(size), binsize(binsize)
{
  filename=new char[strlen(fname)+1];
  strcpy(filename,fname);
...
void histogram::binner(){
  ...
  cout << "max: " << max << ", min: " << min << ", bins: " << bins << ", meansquare: " << meansquare << ", mean: " << mean << ", sdev: " << sdev << endl;
};
...
void histogram::print_custom(){
  ...
  /* } */
  /* fclose(binout); */
  std::cout << "mean : " << mean << ", sdev: " << sdev << std::endl;
};
